...
export const main: T.ExpectedExports.main = async (effects: T.Effects) => {
  ...
  const testSyncthingStillStarting = async () =>
    /no such file or directory/.test(await runCm(` syncthing cli show system`)) ||
    /connection refused/.test(await runCm(` syncthing cli show system`)) ||
    /connection refused/.test(await runCm(` syncthing cli config gui user get`));
  ...
  await runCm(`syncthing cli config gui user set -- $(yq eval '.username' /root/start9/config.yaml)`);
  await runCm(`syncthing cli config gui password set -- $(yq eval '.password' /root/start9/config.yaml)`);
